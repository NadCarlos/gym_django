    ...
    def __call__(self, execute, sql, params, many, context):
        ...
        try:
            ...
            if duration_ms >= self.slow_query_ms:
                logger.warning(
                    "[DB-WRITE] slow_sql request_id=%s view=%s user_id=%s "
...
class WriteDatabaseInstrumentationMiddleware:
    """Instrument write requests and turn InnoDB lock errors into retryable 503s."""

    def __init__(self, get_response):
        self.get_response = get_response
        self.enabled = getattr(settings, "WRITE_DB_INSTRUMENTATION_ENABLED", True)
        self.slow_query_ms = getattr(settings, "DB_SLOW_QUERY_MS", 250)

    def __call__(self, request):
        if not self.enabled or request.method not in WRITE_METHODS:
